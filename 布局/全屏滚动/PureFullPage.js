...
class PureFullPage {
    ...
    //判断,向上滚动
    goUp(){
        console.log('up---',this.container.offsetTop);
        // 只有页面顶部还有页面时页面向上滚动
    ...
    //判断，向下滚动
    goDown(){
        console.log('down---',this.container.offsetTop);
        //只有页面底部还有页面时，页面才能向下滚动。用只有两个页面来类比就行了
        if(-this.container.offsetTop <= this.viewHeight * (this.pagesNum - 2)){
            //重新指定当前页面距试图顶部的距离 currentPosition，实现全屏滚动
            this.currentPosition = this.currentPosition - this.viewHeight;
